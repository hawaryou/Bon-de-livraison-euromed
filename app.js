...
function setupSignature(canvasId, clearButtonId){
  const c=$(canvasId),ctx=c.getContext('2d'); let drawing=false,last=null;
  function resize(){
  ...
  $(clearButtonId).onclick=()=>{ctx.clearRect(0,0,c.width,c.height)};
  return c;
}
const signature=setupSignature('signature','clearSignature');
const technicianSignature=setupSignature('technicianSignature','clearTechnicianSignature');
...
function collect(){
  ...
  return {
    ...
    observations:$('observations').value.trim(), lines, signature:signature.toDataURL('image/png'), technicianName:$('technicianName').value.trim(), technicianSignature:technicianSignature.toDataURL('image/png'),
...
async function buildPdf(data){
  ...
  const blue=[7,125,181], navy=[18,59,90], pale=[231,244,247], lineColor=[193,215,223], text=[28,52,66], muted=[92,117,130];
  let page=1;
  ...
  pdf.setDrawColor(...lineColor);pdf.setFillColor(250,253,254);pdf.roundedRect(margin,y,contentW,30,2,2,'FD');
  ...
  const headerH=9;pdf.setFillColor(...pale);pdf.setDrawColor(...lineColor);pdf.rect(12,y,186,headerH,'FD');
  ...
  data.lines.forEach((line,idx)=>{
    ...
    pdf.setDrawColor(...lineColor);pdf.rect(12,y,186,h);[45,63,128].forEach(x=>pdf.line(x,y,x,y+h));
  ...
  if(data.destinataireAbsent){
    ...
    const boxH=50, gap=6, leftW=100, rightX=118, rightW=80;
    pdf.setFillColor(248,251,252);pdf.setDrawColor(...lineColor);pdf.roundedRect(12,y,leftW,boxH,2,2,'FD');pdf.setFont('helvetica','bold');pdf.setFontSize(8);pdf.setTextColor(...navy);pdf.text('PHOTO DU LIEU',62,y+7,{align:'center'});try{pdf.addImage(data.deliveryPhoto,'JPEG',14,y+10,96,37,{compression:'MEDIUM'});}catch(e){}
    pdf.roundedRect(rightX,y,rightW,boxH,2,2,'FD');pdf.text('TECHNICIEN / INSTALLATEUR',rightX+rightW/2,y+7,{align:'center'});pdf.setFont('helvetica','normal');pdf.setFontSize(8);pdf.setTextColor(...text);pdf.text(pdf.splitTextToSize(techName,70).slice(0,2),rightX+5,y+13);pdf.setDrawColor(...lineColor);pdf.rect(rightX+5,y+17,70,27);try{pdf.addImage(data.technicianSignature,'PNG',rightX+7,y+19,66,23);}catch(e){}y+=boxH+7;
  }else{
    const gap=6, boxW=90, rightX=108;
    pdf.setFillColor(248,251,252);pdf.setDrawColor(...lineColor);pdf.roundedRect(12,y,boxW,58,2,2,'FD');pdf.roundedRect(rightX,y,boxW,58,2,2,'FD');
    pdf.setFont('helvetica','bold');pdf.setFontSize(9);pdf.setTextColor(...navy);pdf.text('SIGNATURE DU CLIENT',57,y+7,{align:'center'});pdf.text('TECHNICIEN / INSTALLATEUR',153,y+7,{align:'center'});
    pdf.setFont('helvetica','normal');pdf.setFontSize(8);pdf.setTextColor(...text);pdf.text(pdf.splitTextToSize(techName,78).slice(0,2),114,y+14);
    pdf.setDrawColor(...lineColor);pdf.rect(17,y+12,80,39);pdf.rect(113,y+17,80,30);
    try{pdf.addImage(data.signature,'PNG',19,y+14,76,35);}catch(e){}
    try{pdf.addImage(data.technicianSignature,'PNG',115,y+19,76,26);}catch(e){}
    y+=64;
  }
