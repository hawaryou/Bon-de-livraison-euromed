...
$('downloadPdf').onclick=async()=>{if(!currentData)return;try{if(!currentPdfBase64)currentPdfBase64=await pdfBase64(currentData);const a=document.createElement('a');a.href='data:application/pdf;base64,'+currentPdfBase64;a.download=filename(currentData);a.click()}catch(e){$('sendStatus').textContent='Impossible de générer le PDF.'}};
