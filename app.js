...
async function buildPdf(data){
  ...
  pdf.setFont('helvetica','normal'); pdf.setTextColor(31,41,55); pdf.setDrawColor(23,59,99); pdf.setLineWidth(.8);
  pdf.line(margin,31,pageW-margin,31);
  pdf.setFontSize(22); pdf.setTextColor(23,59,99); pdf.setFont('helvetica','bold'); pdf.text('BON DE LIVRAISON',margin,20);
  pdf.setFontSize(9); pdf.setTextColor(100,116,139); pdf.setFont('helvetica','normal'); pdf.text('Document de livraison — EuroMed',margin,26);
  try{const logo=await imageData('euromed-logo.jpeg'); pdf.addImage(logo,'JPEG',151,8,45,16);}catch(e){}
  pdf.setFontSize(8);pdf.setTextColor(51,65,85); pdf.text(`Date : ${formatDate(data.date)}`,196,6,{align:'right'});
  pdf.text('+33.327.64.34.99',196,27,{align:'right'}); pdf.text('www.euromed-materiel-medical.com',196,30,{align:'right'}); pdf.text('117 rue de Maubeuge F-59620 Aulnoye-Aymeries',196,33,{align:'right'});

  let y=41; sectionPdf(pdf,'DESTINATAIRE',margin,y,182); y+=7;
  ...
  pdf.text(`E-mail : ${data.emailClient||'—'}`,margin+96,y+17); y+=24;

  sectionPdf(pdf,'MARCHANDISES LIVRÉES',margin,y,182); y+=9;
  const col=[margin,margin+38,margin+58,margin+122,margin+181];
  pdf.setFillColor(233,240,246);pdf.setDrawColor(184,194,204);pdf.setFont('helvetica','bold');pdf.setFontSize(8);
  pdf.rect(margin,y,182,8,'FD'); pdf.text('Nom',margin+19,y+5,{align:'center'}); pdf.text('Quantité',margin+48,y+5,{align:'center'}); pdf.text('Dénomination du produit livré',margin+62,y+5); pdf.text('N° de série / lot',margin+125,y+5);
  y+=8;pdf.setFont('helvetica','normal');
  data.lines.forEach(line=>{
    const h=9; pdf.rect(margin,y,182,h); col.slice(1,-1).forEach(x=>pdf.line(x,y,x,y+h));
    pdf.text(pdf.splitTextToSize(line.name||'',34).slice(0,1),margin+3,y+5);
    pdf.text(String(line.quantity||''),margin+48,y+5,{align:'center'});
    pdf.text(pdf.splitTextToSize(line.product||'',58).slice(0,1),margin+60,y+5);
    pdf.text(pdf.splitTextToSize(line.serial||'',54).slice(0,1),margin+124,y+5); y+=h;
  });
  y+=8; sectionPdf(pdf,'LIVRAISON',margin,y,182); y+=9;
  if(data.observations){pdf.setFont('helvetica','bold');pdf.text('Observations :',margin+3,y);pdf.setFont('helvetica','normal');const obs=pdf.splitTextToSize(data.observations,175);pdf.text(obs,margin+3,y+5);y+=Math.min(30,obs.length*4)+8;}
  ...
  if(data.destinataireAbsent){
    pdf.setFont('helvetica','bold');pdf.setFontSize(11);pdf.text('DESTINATAIRE ABSENT',105,y,{align:'center'}); y+=5;
    try{pdf.addImage(data.deliveryPhoto,'JPEG',65,y,80,55);}catch(e){} pdf.rect(60,y,90,59); y+=64;
  }else{
    pdf.setFont('helvetica','bold');pdf.setFontSize(12);pdf.text('Signature du client',105,y,{align:'center'});y+=3;
    try{pdf.addImage(data.signature,'PNG',65,y,80,38)}catch(e){} pdf.rect(60,y,90,42); y+=48;
  }
...
async function loadGithubDocuments(){
  ...
  box.innerHTML='<div class="no-docs">Chargement des notices et protocoles…</div>';
  if(!window.GITHUB_OWNER || window.GITHUB_OWNER==='A_REMPLACER' || !window.GITHUB_REPO){
    box.innerHTML='<div class="no-docs">Configurez le dépôt GitHub dans <code>config.js</code>.</div>'; return;
  }
  try{
    const res=await fetch(githubApiUrl(),{headers:{Accept:'application/vnd.github+json'}});
    if(!res.ok) throw new Error(`GitHub (${res.status})`);
    const files=await res.json();
    DOCUMENTS=files.filter(f=>f.type==='file' && /\.pdf$/i.test(f.name)).sort((a,b)=>a.name.localeCompare(b.name,'fr')).map(f=>({
      name:f.name.replace(/\.pdf$/i,''),
      path:f.path,
      url:`https://raw.githubusercontent.com/${window.GITHUB_OWNER}/${window.GITHUB_REPO}/${window.GITHUB_BRANCH}/${f.path.split('/').map(encodeURIComponent).join('/')}`
    }));
    box.innerHTML='';
    if(!DOCUMENTS.length){box.innerHTML='<div class="no-docs">Aucun PDF trouvé dans le dossier GitHub configuré.</div>';return}
    DOCUMENTS.forEach((doc,i)=>{
      const lab=document.createElement('label');
      lab.innerHTML=`<input type="checkbox" class="doc-check" data-index="${i}"> ${escapeHtml(doc.name)}`;
      box.appendChild(lab);
    });
  }catch(e){
    console.error(e);
    box.innerHTML=`<div class="no-docs error">Impossible de charger les documents GitHub : ${escapeHtml(e.message)}.</div>`;
  }
}
loadGithubDocuments();
...
async function compressPhoto(file){
  ...
  const img=await imageData(dataUrl);
  const max=1600, scale=Math.min(1,max/Math.max(img.naturalWidth||img.width,img.naturalHeight||img.height));
  const canvas=document.createElement('canvas'); canvas.width=Math.round((img.naturalWidth||img.width)*scale); canvas.height=Math.round((img.naturalHeight||img.height)*scale);
  const ctx=canvas.getContext('2d'); ctx.drawImage(img,0,0,canvas.width,canvas.height);
  return canvas.toDataURL('image/jpeg',.78);
}
$('destinataireAbsent').onchange=async()=>{
  if(!$('destinataireAbsent').checked){$('deliveryPhoto').value='';delete $('deliveryPhoto').dataset.dataUrl;$('photoPreview').classList.add('hidden');$('photoStatus').textContent='';return;}
  $('photoStatus').textContent='Ouverture de l’appareil photo…';
  $('deliveryPhoto').click();
};
$('deliveryPhoto').onchange=async()=>{
  const file=$('deliveryPhoto').files && $('deliveryPhoto').files[0]; if(!file)return;
  try{const dataUrl=await compressPhoto(file);$('deliveryPhoto').dataset.dataUrl=dataUrl;$('photoPreview').src=dataUrl;$('photoPreview').classList.remove('hidden');$('photoStatus').textContent='✓ Photo enregistrée et ajoutée au bon de livraison.';}
  catch(e){console.error(e);$('photoStatus').textContent='Impossible de traiter la photo.';}
};
